...
export async function buildHelmModule({ ctx, module, log }: BuildModuleParams<HelmModule>): Promise<BuildResult> {
  ...
  const values = safeLoad(await helm(ctx, log, "inspect", "values", chartPath)) || {}
...
async function fetchChart(ctx: PluginContext, log: LogEntry, module: HelmModule) {
  const buildPath = module.buildPath
